...
def add_product(p_name: str, purchasing_rate, weight, weight_unit: str, p_desc: str, category: str, Dimension: str, p_img: str, p_qr: str, placement_WR: str, updated_by: str, last_update_by, quantity, quantity_unit: str, fragile, lower_bound, sold_in_last_30_days, sellinng_rate):
    p_id = "P" + str(Database_common_operations.generate_id())
    query = f"insert into Product values('{p_id}','{p_name}',{purchasing_rate},'{weight}','{weight_unit}','{p_desc}','{category}','{Dimension}','{p_img}','{p_qr}','{placement_WR}','{updated_by}','{last_update_by}','{quantity}','{quantity_unit}','{fragile}','{lower_bound}','{sold_in_last_30_days}',{sellinng_rate});"
    res = Database_common_operations.run_query(query)
    if res:
        print("PRODUCT ADDED!")
...
def getMainProductData():
    ...
    query = f"""
        select P_id, P_name, Purchase_Rate, Weight, Category, Quantity, Lower_Bound from Product;
...
def unsetOrderFullfill(oid: str):
    query = f"""
        UPDATE Order_t
        SET Completed_ = 'false'
        where Order_id = '{oid}';
    """
    res = Database_common_operations.run_query(query)
    if res:
